		...
		if(isset($_POST['submit'])) {
			...
			if($guest != "" && $email && $phone != "" && $date_res != "" && $time != "" && $suggestions != "") {
				...
				if($check->num_rows) {
					...
					$insert = $db->query("INSERT INTO reservation(u_id, no_of_guest, email, phone, date_res, time, suggestions) VALUES('".$_SESSION['user_id']."', '".$guest."', '".$email."', '".$phone."', '".$date_res."', '".$time."', '".$suggestions."')");
					...
					if($insert) {
						...
						$msg = "<p style='padding: 15px; color: green; background: #eeffee; font-weight: bold; font-size: 13px; border-radius: 4px; text-align: center;'>Reservation placed successfully. Please Wait for your Table number.</p>";
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="hr_book_form">
			...
			<div class="left">
				...
				<div class="form_group">
					...
					<label>Email</label>
					<input type="email" name="email" placeholder="Enter your email" value="<?php echo isset($_SESSION['user_email']) ? $_SESSION['user_email'] : ''; ?>" required readonly>
