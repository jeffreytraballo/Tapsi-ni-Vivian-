<?php 
	...
	$per_page = 20;
		...
		$sql = "SELECT reservation.*, tables.table_name 
			FROM reservation 
			LEFT JOIN tables ON reservation.table_id = tables.table_id 
			ORDER BY reservation.reserve_id ASC LIMIT $start, $per_page";
	...
	$reserve = $db->query($sql);
		...
		if($reserve->num_rows) {
			...
			while($row = $reserve->fetch_assoc()) {
				...
				// Disable the edit button if the status is "confirmed"
				$editButton = ($status === 'Confirmed') 
					? "<button class='edit-btn' disabled style='cursor:not-allowed; opacity:0.6;'>
						   <i class='pe-7s-edit'></i> Edit
					   </button>" 
					: "<a href='edit_reservation.php?edit=".$reserve_id."' class='edit-btn'>
						   <i class='pe-7s-edit'></i> Edit
					   </a>";
				...
				$result .=  "<tr>
								...
								<td> 
									$editButton
									<a href='delete_reservation.php?delete=$reserve_id' class='btn btn-danger btn-sm' 
