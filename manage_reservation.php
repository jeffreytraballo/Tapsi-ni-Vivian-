<?php 
	...
	$per_page = 20;
		...
		$reserve = $db->query("SELECT * FROM reservation  where u_id = '".$_SESSION['user_id']."' LIMIT $start, $per_page");
		...
		if($reserve->num_rows) {
			...
			while($row = $reserve->fetch_assoc()) {
				...
				$result .=  "<tr>
								...
								<td>$suggestions</td>
								<td>$status</td>
								<td> <a href='manage_reservation.php?delete=".$reserve_id."' onclick='return check();'><i class='pe-7s-close-circle'></i></a></td>
							</tr>";
			...
			$result = "<p style='color:green; padding: 10px; background: #ffeeee;'>You have no reservation yet</p>";
	...
	if(isset($_GET['delete'])) {
		...
		if($delete != "") {
			...
			if($sql) {
				...
				header("Location: " . $_SERVER['PHP_SELF']); // Redirect instead of infinite refresh
				exit;
        ...
        <footer class="footer">
            <div class="container-fluid">
                ...
                <p class="copyright pull-right">
                    &copy; 2025 <a href="index1.php" style="color: #FF5722;">Tapsi ni Vivian at Bulaluhan</a>
                </p>
            </div>
        </footer>
